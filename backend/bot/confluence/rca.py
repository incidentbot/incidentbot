...
class IncidentRootCauseAnalysis:
    ...
    def create(self) -> str:
        ...
        try:
            attachments = (
                ...
                .filter(IncidentLogging.incident_id == self.incident_id)
                .all()
            )
        except Exception as error:
            logger.error(
                f"Error retrieving pinned items during RCA creation: {error}"
            )
        finally:
            Session.close()
            Session.remove()
        ...
        body = self.__render_rca_html(
            ...
            timeline=self.__generate_timeline(
                log.read(incident_id=self.incident_id)
            ),
            attachments=self.__generate_pinned_items(attachments),
        )
        # Create rca doc
    ...
    def __generate_pinned_items(self, items: IncidentLogging) -> str:
        if len(items) == 0:
            return "<p>No items were pinned for this incident.</p>"
        all_items_formatted = ""
        for item in items:
            if item.img == b"":
                all_items_formatted += f"<blockquote><p><strong>{item.user} @ "
                +f"{item.ts} - </strong> {item.content}</p></blockquote><p />"
        return all_items_formatted

    def __generate_timeline(self, items: List[Dict]) -> str:
        if len(items) == 0:
            return """
    <tr>
        <td>
