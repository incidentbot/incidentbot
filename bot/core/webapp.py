...
from __main__ import flask_app as app, config
...
def enable_user_post():
    if flask_login.current_user.is_admin == True:
        ...
        try:
            ...
            flash(f"Error: {error}")
        if not (user):  # if a user is not found, we say so d
            flash(f"User {email} doesn't exist.")
            return redirect(url_for("adminpanel"))
        try:
            db.db_user_enable(
...
def create_incident_post():
    ...
    try:
        incident.create_incident(internal=False, request_parameters=request_parameters)
...
def set_status_post():
    ...
    inc_actions.set_incident_status(override_dict=params)
    return redirect(url_for("incidents"))
...
def set_severity_post():
    ...
    inc_actions.set_severity(override_dict=params)
    return redirect(url_for("incidents"))
