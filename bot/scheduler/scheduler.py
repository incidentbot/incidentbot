...
def scheduled_reminder_message(channel_name: str, channel_id: str, severity: str):
    ...
    """
    incident = db.db_read_incident(incident_id=channel_name)
    ...
    if last_update_sent == None:
        ...
        if been_30_minutes:
            try:
                slack_client.slack_web_client.chat_postMessage(
                    channel=channel_id,
...
def scrape_for_aging_incidents():
    ...
    # than the max age
    open_incidents = db.db_read_open_incidents()
    formatted_incidents = []
...
process.scheduler.add_job(
    id=f"scrape_for_aging_incidents",
    func=scrape_for_aging_incidents,
    trigger="interval",
    name=f"Look for stale incidents and inform the digest channel",
    days=2,
    replace_existing=True,
)
