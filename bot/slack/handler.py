...
def handle_mention(body, say, logger):
    ...
    if "help" in message:
        ...
        if config.pagerduty_api_token != "" and config.pagerduty_api_username != "":
            ...
            say(
                text="The PagerDuty username and API token for your account have not been provided. I am unable to provide on-call information as a result."
            )
...
def handle_incident_add_on_call(ack, body):
    logger.debug(body)
    ack()
    inc_actions.add_on_call_to_channel(action_parameters=parse_action(body))
