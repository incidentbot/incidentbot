...
export default function OnCallTableOG(props) {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - props.data.data.length) : 0;
        ...
        <TableContainer>
          <Table sx={{ minWidth: 850 }} aria-labelledby="tableTitle" size="small">
            <EnhancedTableHead
              ...
              rowCount={props.data.data.length}
            />
            <TableBody>
              {props.data.data.length > 0 ? (
                stableSort(props.data.data, getComparator(order, orderBy))
